...
class MyEngine(Engine):
    ...
    def train(engine, mini_batch):
        # You have to reset the gradient of all model parameters
    ...
    def attach(train_engine, validation_engine, verbose=VERBOSE_BATCH_WISE):
        ...
        # If the verbosity is set, progress bar would be shown for mini_batch iterations.
    ...
    def check_best(engine):
        ...
        if loss <= engine.best_loss:  # if current epoch returns lower validation loss,
            engine.best_loss = loss  # Update lowest validation
...
class Trainer():
    ...
    def train(self, model, crit, optimizer, train_loader, valid_loader):
        train_engine = MyEngine(MyEngine.train, model,
                                crit, optimizer, self.config)
        validation_engine = MyEngine(
            MyEngine.validate, model, crit, optimizer, self.config)
